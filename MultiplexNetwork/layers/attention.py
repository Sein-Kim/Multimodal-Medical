...
class Attention(nn.Module):
    ...
    def forward(self, feat_pos, feat_neg, summary):
        feat_pos, feat_pos_attn, p = self.attn_feature(feat_pos)
        feat_neg, feat_neg_attn, p_ = self.attn_feature(feat_neg)
        summary, summary_attn = self.attn_summary(summary)

        return feat_pos, feat_neg, summary, p
    ...
    def attn_feature(self, features):
        ...
        p = torch.mean(features_attn,dim=0)
        ...
        return features, features_attn, p
